...
/**
 * Created by 23alot on 13.03.2020.
 */
class ObjLoader(context: Context, file: InputStream) {
    val numFaces: Int
    ...
    init {
        ...
        try {
            val `in` = InputStreamReader(file)
            reader = BufferedReader(`in`)

            // read file until EOF
            var line: String?
            while (reader.readLine().also { line = it } != null) {
                val parts = line?.split(" ")?.toTypedArray() ?: arrayOf<String>("exit")
                when (parts[0]) {
                    "v" -> {
                        // vertices
        ...
        for (face in faces) {
            ...
            var index: Int = 3 * (parts[0].toShort() - 1)
